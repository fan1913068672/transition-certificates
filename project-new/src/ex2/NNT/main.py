...
def f_t(x1, x2):
    ...
    Omega = 0.01
    K = 0.0006
    x1p = x1 + Ts * Omega + 1.69 + Ts * K * dreal.sin(x2 - x1) - 0.532 * Ts * x1 ** 2
    x2p = x2 + Ts * Omega + 1.69 + Ts * K * dreal.sin(x1 - x2) - 0.532 * Ts * x2 ** 2
    return x1p, x2p
...
def f_m(x1, x2):
    ...
    Omega = 0.01
    K = 0.0006
    x1p = x1 + Ts * Omega + 1.69 + Ts * K * math.sin(x2 - x1) - 0.532 * Ts * x1 ** 2
    x2p = x2 + Ts * Omega + 1.69 + Ts * K * math.sin(x1 - x2) - 0.532 * Ts * x2 ** 2
    return x1p, x2p
...
def verify_with_dreal(B_net, dreal_precision=1e-4):
    """
...
def synthesize_barrier_certificate(save_dir="saved_models", max_iterations=2000, train_epochs=1000, train_lr=0.01, dreal_precision=1e-4):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--grid-step", type=float, default=0.0, help="random seed for neural synthesis")
    parser.add_argument("--dreal-precision", type=float, default=1e-4, help="dReal precision for formal verification")
    ...
    print_header(
        ...
        "NNT",
        "transition_safety",
        {"solver_verify": "dreal", "hidden": 15, "max_iter": args.max_iter, "epochs": args.epochs, "lr": args.lr, "seed": args.seed},
    )
    ...
    result = {
        ...
        "solver": {"synth": "adam", "verify": "dreal", "dreal_precision": args.dreal_precision},
