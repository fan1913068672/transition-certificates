...
Intuitively,
\begin{itemize}
\item condition (\ref{btpc1}) guarantees that the certificate $B_{k,l}$ is non-negative at the initial state of the system;

\item condition (\ref{btpc2}) ensures that $B_{k,l}$ remains non-negative along any trajectory of the system;

\item condition (\ref{btpc3}) implies that $B_{k,l}$ decreases by at least $\epsilon$ whenever the state's label matches an accepting transition from $q_k$ to $q_l$, and remains non-increasing otherwise.
\end{itemize}
