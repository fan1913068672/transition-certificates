...
def main() -> int:
    parser = base_parser("ex1/ST test")
    args = parser.parse_args()

    report = make_report("ex1", "ST")
    check_sampled_initial_points(report, None, "ex1", args.x0_samples, args.sim_steps)
    try:
        m = load_local_main(__file__)
        pi = m.PI
        add_check(report, "unsafe boundary", m.in_unsafe(7 * pi / 9) and m.in_unsafe(8 * pi / 9), "unsafe interval mismatch")
        add_check(report, "delta structure", m.delta(0.0, 1) == [1] and m.delta(7 * pi / 9, 1) == [0], "delta mismatch")
        add_check(report, "dynamics finite", math.isfinite(m.f_m(1.0)), "f_m not finite")
        ...
        result_path = Path(args.result) if args.result else Path(__file__).resolve().parent / "res_st_ex1_10000.json"
        if result_path.exists():
            data = json.loads(result_path.read_text(encoding="utf-8"))
            add_check(report, "result type", data.get("certificate_type") == "state_triplet_barrier", "wrong certificate_type")
            add_check(report, "result status", data.get("status") == "proved" and data.get("success") is True, "ex1 ST should be proved")
            ...
            add_check(report, "coefficients", backend.get("coefficients") == [0.0, 0.0, -4.5, 2.0, 0.0], "unexpected ex1 ST coefficients")
