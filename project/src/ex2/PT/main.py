...
PI = 3.1415926
def In_X(x1, x2):
    return x1 >= 0 and x1 <= PI / 9 * 8 and x2 >= 0 and x2 <= PI / 9 * 8

def In_X_Cond(x1_ce, x2_ce):
    return dreal.And(dreal.And(x1_ce >= 0, x1_ce <= PI / 9 * 8), dreal.And(x2_ce >= 0, x2_ce <= PI / 9 * 8))

def In_X0_Cond(x1_ce, x2_ce):
    return dreal.And(dreal.And(x1_ce >= 0, x1_ce <= PI / 9), dreal.And(x2_ce >= 0, x2_ce <= PI / 9))
...
def In_Unsafe(x1, x2):
    return x1 >= 5 / 6 * PI and x1 <= 8 / 9 * PI or x2 >= 5 / 6 * PI and x2 <= 8 / 9 * PI

def In_Unsafe_Cond(x1_ce, x2_ce):
    return dreal.Or(dreal.And(x1_ce >= PI / 6 * 5, x1_ce <= PI / 9 * 8),
             dreal.And(x1_ce >= PI / 6 * 5, x1_ce <= PI / 9 * 8))
...
def reachability_10():
    ...
    X1_Samples = step_sample(0, 8 * PI / 9, 0.1)
    X2_Samples = step_sample(0, 8 * PI / 9, 0.1)
    Q_Samples = [0, 1]
    Y_Samples = state_space_product(X1_Samples, X2_Samples, Q_Samples)
    X01_Samples = step_sample(0, PI / 9, 0.1)
    X02_Samples = step_sample(0, PI / 9, 0.1)
    Q0_Samples = [1]
