...
def trace_based_state_safety(initial_state, qu):
...
def trace_based_transition_persistence(initial_state, q1, q2):
...
def main():
    ...
    for i in range(int(a * int(1/s)), int(b*int(1/s))+1):
        x1 = i * s
        for j in range(int(a * int(1/s)), int(b * int (1/s)) + 1):
            x2 = j * s
            flag = trace_based_transition_persistence((x1, x2), 1, 2)
            if flag:
                ce_list.append((x1, x2))
                continue
            flag = trace_based_state_safety((x1, x2), 2)
            if flag:
                ce_list.append((x1, x2))
...
start = time.time()
ce_list = main()
for x1, x2 in ce_list:
    print(x1, x2)
print("counterexample trajectory: ", x1, x2)
end = time.time()
