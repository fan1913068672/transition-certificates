...
def main() -> int:
    ...
    try:
        try:
            ...
            pi = m.PI
            add_check(report, "unsafe region config", m.in_unsafe(5 * pi / 6, 0.0) and m.in_unsafe(0.0, 5 * pi / 6), "Xu definition mismatch")
            add_check(report, "initial region config", m.in_x0(0.0, 0.0) and m.in_x0(pi / 9, pi / 9), "X0 boundary mismatch")
            ...
            add_check(report, "verify backend", case.verify_backend == "z3", f"verify_backend={case.verify_backend}")
            add_check(report, "interval transition relation", case.next_relation_z3 is not None, "missing Z3 interval over-approximation relation")
            ...
            add_check(report, "fallback verify backend", "z3_interval_overapprox" in text, "strict Z3 interval verifier not found in source")
        ...
        if args.result:
            ...
            add_check(report, "result meta", data.get("example") == "ex2" and data.get("method") == "CC", "wrong example/method")
            ...
            add_check(report, "result T_0_0", isinstance(t00, list) and len(t00) == 7, "expected original-paper 7-term template in T_0_0")
            if data.get("success"):
                add_check(report, "result epsilon", isinstance(data.get("epsilon"), (float, int)), "missing epsilon")
