...
def f_t(x1, x2):
    if dreal is None:
        x1p = x1 + TS * OMEGA + 1.69 - 0.532 * TS * x1**2
        x2p = x2 + TS * OMEGA + 1.69 - 0.532 * TS * x2**2
        return x1p, x2p
...
def main() -> None:
    ...
    parser.add_argument("--seed", type=int, default=0, help="recorded only; ST SMT search is deterministic")
    args = parser.parse_args()

    print_header("ex2", "ST", "state_triplet_barrier", {"strict_wong": True, "backend": "z3+dreal", "max_iter": args.max_iter, "dreal_precision": args.dreal_precision, "seed": args.seed})
    start = time.time()
    case = build_case()
