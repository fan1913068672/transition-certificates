    ...
    for epoch in range(epochs):
        ...
        if len(training_data['strict_dec']) > 0:
            ...
            for x1, x2, x1p, x2p in training_data['strict_dec']:
                ...
                # 惩罚 B_curr - B_next < epsilon
                # 即惩罚 B_next > B_curr - epsilon
...
def synthesize_persistence_certificate():
    ...
    """
    print("=" * 70)
    print("STEP 2: Synthesizing TRANSITION PERSISTENCE CERTIFICATE (两层网络)")
    print("网络结构: 输入(2) -> 隐藏层(3) -> 输出(1)")
    print("激活函数: 平方函数 (x²)")
    print("多项式阶数: 4次")
    print("验证工具: dReal (δ-完备性)")
    print("=" * 70)

    # 初始化两层神经网络
    B_net = BarrierNetwork(input_dim=2, hidden_dim=3)  # 隐藏层维度为3
    ...
    # 初始化训练数据
    training_data = {
        'non_inc': [],      # 非接受转移 (VF外) - 非递增
        'strict_dec': []    # 接受转移 (VF内) - 严格递减
    }
    ...
    while iter < MAX_ITER:
        print(f"\n{'='*70}")
        print(f"CEGIS 迭代 #{iter}")
        print(f"{'='*70}")
        print(f"非递增={len(training_data['non_inc'])}, " +
              f"严格递减={len(training_data['strict_dec'])}")
...
if __name__ == "__main__":
    ...
    if success:
        print("\n最终持久性证书:")
        print("-" * 70)
        print(f"网络结构: 2 -> {B_net.fc1.out_features} -> 1")
        print(f"激活函数: 平方函数 (x²)")
        print(f"多项式阶数: 4次")
        print(f"Epsilon (ε): {B_net.get_epsilon():.6f} (固定)")
        print("\n权重矩阵 W1:")
        ...
        print("-" * 70)
        print("\n解释:")
        print("  • B(x1, x2) > 1000 在初始状态 (确保足够的递减空间)")
        print("  • B(x1, x2) 在所有转移上非递增")
        print(f"  • B(x1, x2) 在VF区域内严格递减至少 ε={B_net.get_epsilon():.6f}")
        print("  • 因此，q=1 只能被访问有限多次 ✓")
