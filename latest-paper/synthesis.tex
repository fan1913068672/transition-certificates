...
We briefly comment on completeness. For polynomial templates applied to systems with polynomial dynamics and semialgebraic constraints, the certificate conditions can in principle be discharged via Positivstellensatz-type arguments, which guarantee that a sufficient polynomial certificate of high enough degree exists whenever the property holds under mild regularity conditions. In this setting, iterating over templates of increasing degree provides a theoretical path to completeness, though the required degree may be large in the worst case. For neural network templates, the situation differs: failure of the CEGIS loop to converge indicates that the current architecture lacks the expressiveness to represent a valid certificate, not that no certificate exists. In such cases, enlarging the network, changing the activation function, or switching to a polynomial template of higher degree may resolve the issue.
