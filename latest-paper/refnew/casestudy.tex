...
We conduct two groups of comparisons: (1) polynomial-template CEGIS versus closure certificates, and (2) neural-template CEGIS versus neural closure certificates. The detailed experimental configurations are provided in Appendix~\ref{appendix-experiment}. Table~\ref{tab:results-poly} reports the synthesis times stated in the closure-certificate paper~\cite{murali2024closure} for the three benchmarks and our local state-triplet runs; ``Timeout'' means that no formally verified state-triplet barrier was obtained within the time limit under the reported template. Table~\ref{tab:results-neural} reports our local runs of the neural closure-certificate baseline; there, ``Timeout'' indicates that the synthesis exceeded one hour. As shown in Table~\ref{tab:results-poly}, when suitable templates are selected, our polynomial-template CEGIS method completes synthesis within seconds, whereas the reported closure-certificate baseline requires around 10 minutes for Kuramoto 1D, 1 hour and 50 minutes for Kuramoto 2D, and 1.5 hours for the two-room temperature benchmark. The state-triplet baseline verifies the two Kuramoto cases but times out on the two-room case under the tested template, consistent with its path-blocking conservatism for persistence-style properties. Similarly, as shown in Table~\ref{tab:results-neural}, the neural-network template CEGIS requires approximately 10--20 minutes, which is slower than the polynomial-template version but remains considerably faster than the neural closure-certificate baseline under our tested settings. These comparisons indicate substantial efficiency gains obtained from reducing the search space complexity.
