...
The configuration for the neural closure certificates is summarized in Table~\ref{tab:ncc_config}. A sampling density $\xi$ of 0.01 and a safety margin $\eta$ of 0.01 are used for all systems. The Lipschitz constant is estimated using the spectral norm-based algorithm. For the Kuramoto benchmarks, NCC uses the original state-based automaton for $\mathbf{F}a$; for the two-room benchmark, NCC uses $a \land \mathbf{GF} b$.
