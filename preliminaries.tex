...
Barrier certificates and closure certificates are used to LTL properties \cite{murali2024closure}, respectively. We provide their definitions and related conclusions from prior work.
...
The state-triplet approach\cite{wongpiromsarn2015automata} uses barrier certificates (\ref{SCDEF}) to verify LTL properties. Given an LTL specification $\phi$, it proves that accepting states are unreachable in the NBA for $\neg\phi$ by finding barrier certificates to ensure that runs on $\mathfrak{L}(\mathfrak{S})$ will never visit accepting states, verifying $\mathfrak{S} \models_{\mathfrak{L}} \phi$. Our transition certificates share this reachability-blocking principle but operates on the product system $\mathfrak{S} \times \mathcal{A}$, eliminating the need to enumerate simple paths in the NBA from initial state to accepting state. The state-triplet method need to find barrier certificates for each simple path, a certificate corresponding to a specific state triplet to individually block a simple path. The state-triplet method introduces conservatism: it cannot be applied for systems that satisfy the LTL specification because they cannot be proven safe under this restrictive constraints.

%The state-triplet method requires one certificate for some triplet to block each simple path individually, while we synthesize one certificate per accepting transition start state in $Acc^{start}$. Transition certificates further allows visiting accepting states while proving transitions occur finitely often and reduce conservatism.
...
Existing methods face limitations: closure certificates require searching a large space, making synthesis slow, and the state-triplet method can be overly conservative. In this paper, we propose a new approach to address both issues.
