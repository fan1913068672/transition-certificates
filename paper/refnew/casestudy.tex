...
We evaluate the proposed transition certificate synthesis methods on two benchmark systems. The Kuramoto oscillator benchmarks test safety-style LTL properties in one and two dimensions. The two-room temperature benchmark tests a persistence-style property where safety and persistence certificates are used together. All experiments are conducted on Ubuntu 22.04.3 LTS (Intel i7-9750H, 8 GB RAM). During candidate generation, Z3 solves the constrained optimization problem for polynomial templates. Neural templates are trained with Adam using learning rate 0.001 for 1000 iterations. For formal verification, we use dReal with precision 0.0001 for the transcendental $\sin$ terms in the Kuramoto oscillator and Z3 for the temperature model.
...
% 如何将安全性验证转化为LTL验证
We consider the LTL specification $\phi = \mathbf{G}\neg a$ over the atomic proposition set $AP = \{a\}$, with the labeling function defined as $\mathfrak{L}(x) = \{a\}$ for $x \in \mathcal{X}_u$ and $\mathfrak{L}(x) = \emptyset$ otherwise. Across the five implementations PT, ST, CC, NCC, and NNT, we use a unified semantic target: trajectories starting from $\mathcal{X}_0$ must avoid $\mathcal{X}_u$ forever. For PT and NNT, we use the transition-based negated automaton for $\neg \phi = \mathbf{F} a$, shown in Figure~\ref{pic-tbnba1}. CC and NCC use the corresponding state-based automaton for $\mathbf{F}a$. ST uses the strengthened state-based encoding $i \land \mathbf{F}u$, where $i$ labels $\mathcal{X}_0$ and $u$ labels $\mathcal{X}_u$, so that the accepting paths contain the state triplets required by the path-blocking construction; over trajectories generated from $\mathcal{X}_0$, this encoding is equivalent to $\mathbf{F}a$.
...
\end{equation}
To maintain consistency with the 1D analysis, we define the labeling function such that $\mathfrak{L}(x_1, x_2) = \{a\}$ if $(x_1, x_2) \in \mathcal{X}_u$ and $\emptyset$ otherwise. PT and NNT use the transition-based negated automaton for $\mathbf{F} a$, CC and NCC use the state-based automaton for $\mathbf{F}a$, and ST uses the strengthened state-based encoding $i \land \mathbf{F}u$, with $i$ denoting $\mathcal{X}_0$ and $u$ denoting $\mathcal{X}_u$. These encodings coincide on all trajectories generated from $\mathcal{X}_0$. The verification is therefore performed against the same negated behavior as in the 1D case.
...
Consider the set of atomic propositions $AP = \{a, b\}$, where the labeling function is defined as $\mathfrak{L}(x_1, x_2) = \{a\}$ for $(x_1, x_2) \in \mathcal{X}_0$, and $b \in \mathfrak{L}(x_1, x_2)$ for $(x_1, x_2) \in \mathcal{X}_{VF}$. The system is required to satisfy the LTL specification $\varphi = a \Rightarrow \mathbf{FG}\neg b$. For all five implementations PT, ST, CC, NCC, and NNT, we use the same negated semantic target $\neg \varphi = a \land \mathbf{GF} b$. The corresponding NBA is illustrated in Figure~\ref{pic-tbnba2}. To demonstrate the complementary roles of transition safety certificates and transition persistence certificates, we synthesize both types of certificates for this benchmark.
...
Consider the transition safety certificate template and transition persistence certificate template(seen in Appendix~\ref{appendix-experiment}). We simultaneously synthesize them. For the transition safety certificate, we collect 16 samples for $D_0$, 196 for $D_u$, 588 for $D_{\mathcal{X}}$. For the transition persistence certificate, we collect 16 samples for $D_0$ and 196 for $D_{\mathcal{X}}$. We obtain a valid transition persistence certificate $$B_{1,1}(x_1, x_2) = 27 I_{[20,26]^2}(x_1, x_2) - x_1 I_{[20,26]^2}(x_1, x_2)$$ and $\epsilon = 0.25$ in 2.68 seconds.

We synthesize the transition safety certificate $B_1(x_1, x_2, q)$ for $q_1$ and the transition persistence certificate $B_{1,1}(x_1, x_2)$ for $Acc^{1,1}$. The template for $B_1$ has architecture 3-10-1, with inputs $(x_1,x_2,q)$, 10 ReLU hidden units, and one output. The template for $B_{1,1}$ has architecture 2-3-1, with inputs $(x_1,x_2)$, three hidden units with squared activation, and one output. For the transition persistence certificate, we collect 100 samples from $D_{\mathcal{X}}$. For the transition safety certificate, we collect 4 samples from $D_0$, 9 from $D_u$, and 18 from $D_{\mathcal{X}}$. The CEGIS loop terminates in 1461.91 seconds. The valid certificates are provided in the appendix.

\subsection{Comparison}
We primarily compare our approach with closure certificates~\cite{murali2024closure} and neural closure certificates~\cite{nadali2024neural}, as they represent recent developments in abstraction-free LTL verification. We also report local runs of the state-triplet method~\cite{wongpiromsarn2015automata} as a foundational conservative baseline. The state-triplet method is more restrictive because it attempts to block all simple paths to accepting states, while closure certificates~\cite{murali2024closure} were specifically developed to provide more general verification conditions than the state-triplet method. The core idea of closure certificates lies in characterizing an over-approximation of the transition closure, so closure certificates for LTL verification are defined over $\mathcal{X} \times \mathcal{X} \times Q \times Q$. In contrast, our method employs transition safety certificates to directly filter out unreachable automaton states or uses transition persistence certificates to prove that the accepting transitions occur finitely often, so the search space of transition certificates is at most $\mathcal{X} \times Q \times Q$. This allows us to reduce the search space from $\mathcal{X} \times \mathcal{X} \times Q \times Q$ to $\mathcal{X} \times Q \times Q$ and improves synthesis efficiency. For neural templates, we adopt the CEGIS framework, whereas the neural closure certificate approach relies on sampling and posteriori verification: after sampling the state space with a given density $\epsilon$ and safety margin $\eta$, the method trains until the loss converges to zero and then performs a posteriori verification to determine whether the synthesized certificate is valid.

We conduct two groups of comparisons. The first compares polynomial-template CEGIS with closure certificates. The second compares neural-template CEGIS with neural closure certificates. The detailed experimental configurations are provided in Appendix~\ref{appendix-experiment}. Table~\ref{tab:results-poly} reports the synthesis times stated in the closure-certificate paper~\cite{murali2024closure} for the three benchmarks and our local state-triplet runs. There, ``Timeout'' means that no formally verified state-triplet barrier was obtained within the time limit under the tested template. Table~\ref{tab:results-neural} reports our local runs of the neural closure-certificate baseline. In that table, ``Timeout'' means that synthesis exceeded one hour without a verified certificate. Under these settings, the polynomial-template transition-certificate runs finish within seconds, while the reported closure-certificate baseline takes minutes to hours on the same benchmark family. The state-triplet baseline verifies the two Kuramoto cases but times out on the two-room case, which is consistent with its path-blocking conservatism for persistence-style properties. The neural-template transition-certificate runs take approximately 10--20 minutes. These results support an efficiency claim for the tested templates and benchmarks, while timeout rows are treated as bounded reproduction outcomes rather than failed theorems.

\begin{table}[t]
\centering
\caption{Polynomial-template synthesis time comparison in seconds. ``Timeout'' means that no verified certificate was obtained within the stated budget for that baseline configuration.}
\label{tab:results-poly}
\begin{tabular}{lccc}
\hline
\textbf{Benchmark} & \textbf{Transition Certificates} & \textbf{State Triplet~\cite{wongpiromsarn2015automata}} & \textbf{Closure Certificate~\cite{murali2024closure}} \\
\hline
Kuramoto 1D & 1.76 & 0.10 & 600 \\
Kuramoto 2D & 1.72 & 1.13 & 6600 \\
Two-Room Temperature & 2.68 & Timeout & 5400 \\
\hline
\end{tabular}
\end{table}
\begin{table}[t]
\centering
\caption{Neural-template synthesis time comparison in seconds. ``Timeout'' records bounded synthesis exhaustion, not a theorem failure.}
\label{tab:results-neural}
\begin{tabular}{lcc}
\hline
\textbf{Benchmark} & \textbf{Transition Certificates} & \textbf{Neural Closure Certificate (local run)} \\
\hline
