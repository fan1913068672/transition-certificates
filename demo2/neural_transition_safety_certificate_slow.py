    ...
    for epoch in range(epochs):
        ...
        if len(training_data['init']) > 0:
            ...
            for x01, x02, q0 in training_data['init']:
                B_init = B_net(torch.tensor(x01), torch.tensor(x02), torch.tensor(q0))
        ...
        if len(training_data['unsafe']) > 0:
            ...
            for xu1, xu2, qu in training_data['unsafe']:
                B_unsafe = B_net(torch.tensor(xu1), torch.tensor(xu2), torch.tensor(qu))
        ...
        if len(training_data['transition']) > 0:
            ...
            for x1, x2, q, x1p, x2p, qp in training_data['transition']:
                B_curr = B_net(torch.tensor(x1), torch.tensor(x2), torch.tensor(q))
                B_next = B_net(torch.tensor(x1p), torch.tensor(x2p), torch.tensor(qp))
                # If B(x1, x2, q) >= 0, then B(x1', x2', q') should >= 0
